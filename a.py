...
def get_custom_prompt():
    ...
    Ensure your response includes all available and relevant content from the document and previous interactions to fully address the user's query. If the exact answer is not found within the data, respond with:
...
async def main():
    ...
    while True:
        ...
        try:
            history = "\n".join(
                [f"Q: {interaction['query']}\nA: {interaction['response']}" for interaction in interactions]
            )
            result = qa_chain.invoke({"history":history,"query": user_query,"context":""})
