...
def extract_text_from_pdf(pdf_path: str, chunk_size=2000, overlap=500) -> list:
...
def save_query_response_to_mongodb(query, response):
    """Save the user query and model response to MongoDB."""
    try:
        document = {
            "query": query,
            "response": response
        ...
        print("Query and response saved to MongoDB.")
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        try:
            ...
            save_query_response_to_mongodb(user_query, response)
