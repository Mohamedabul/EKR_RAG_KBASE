...
def filter_documents(texts):
    embedding_model1 = SentenceTransformerEmbeddings(model=SentenceTransformer('all-MiniLM-L6-v2'))
    embeddings_filter = EmbeddingsClusteringFilter(
        embeddings=embedding_model1,
        num_clusters=min(len(texts) // 2, 8),  
        num_closest=min(len(texts) // 4, 3),    
        threshold=0.85                          
    )
...
def extract_text(file):
    ...
    if file_extension == '.pdf':
        ...
        raise ValueError("Unsupported file format. Please use PDF, CSV, or text documents.")
