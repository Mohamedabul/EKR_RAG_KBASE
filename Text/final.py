...
def create_faiss_index(content_chunks):
    ...
    faiss_index = FAISS.from_texts(
        ...
        metadatas=[{"source": "text_file", "index": i} for i in range(len(content_chunks))]
    )
    return faiss_index
...
def save_summary_and_interaction_to_mongodb(file_summary, interactions):
    """Save the file summary and interactions to MongoDB."""
    document = {
...
async def main():
    ...
    try:
        ...
        file_summary = await summarize_large_text(text_content)
        print("Summary of Text:\n", file_summary)
    except Exception as e:
        print(f"Failed to extract or summarize text content: {e}")
        return

    content_chunks = chunk_text(text_content, max_length=2000)
    try:
    ...
    save_summary_and_interaction_to_mongodb(file_summary, interactions)
